...
parser.add_argument('--p', type=int, default=0,  help='how many stages')
...
if args.type == 'tinyimagenet':
    test_loader = dataset.tinyimagenet(batch_size=args.batch_size)[1]
    # load_file = torch.load(args.baseline_model_pth, map_location='cpu')
    # print('Next is printed')
    model = detector_net.Net() #detector_net_tiny.Net()
    # print(net)
...
lr_list = [0.003, 0.001, 0.001, 2.5, 3.0]
...
optimizer = optim.SGD(model.parameters(), lr=args.lr, weight_decay=5e-4, momentum=0.5)
...
f.write(f'lambda_clean {lambda_clean}, lambda_adv {lambda_adv}, lr {args.lr}')
print(f'lambda_clean {lambda_clean}, lambda_adv {lambda_adv}, lr {args.lr}')
n_batches = int(500 * args.p * 0.01)
for epoch in range(args.epochs):
    print(f'epoch : {epoch}')
    model.train()
    # random.shuffle(train_loader)
    # print("training phase")
    l = ['set_task_'+args.task+'_99', 'set_task_'+args.task+'_199', 'set_task_'+args.task+'_299', 'set_task_'+args.task+'_399', 'set_task_'+args.task+'_499']
    count_batches = 0
    for ik in l:
        print(ik)
        train_loader = torch.load('./tinyimagenet/' + ik, map_location='cuda')
        random.shuffle(train_loader)
        for batch_idx, (data, target, y) in enumerate(train_loader):
            # print(batch_idx)
            if count_batches < n_batches:
                # print(count_batches)
    ...
    if epoch % args.test_interval == 0:
        ...
        print(f'best sep {best_sep}, best loss {best_loss}')
